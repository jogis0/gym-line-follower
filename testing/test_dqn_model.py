...
def _print_run_config(model_path: Path) -> None:
    ...
    print(f"  action_names     : {cfg.action_names}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run the deterministic 10-track evaluation on a trained DQN model.")
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED,
                        help="Training seed used to locate the model directory.")
    parser.add_argument("--model-path", type=Path, default=None,
                        help="Path to the .zip model. Default: models/DQN/seed_<seed>/dqn_turtlebot3.zip")
    ...
    if not args.model_path.exists():
        raise FileNotFoundError(
            ...
            f"Train one first with: python models/turtlebot_dqn.py --train --seed {args.seed}"
        )
    ...
    model = DQN.load(str(args.model_path), env=eval_venv)
