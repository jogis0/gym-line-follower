...
def main():
    parser = argparse.ArgumentParser(
        description="Run the deterministic 10-track evaluation on a trained RecurrentPPO model.")
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED,
                        help="Training seed used to locate the model directory.")
    parser.add_argument("--model-path", type=Path, default=None,
                        help="Path to the .zip model. Default: D:/RecurrentPPO/seed_<seed>/recurrent_ppo_turtlebot3.zip")
    ...
    if args.model_path is None:
        args.model_path = best_model_path_for(args.seed) if args.best else model_path_for(args.seed)

    if not args.model_path.exists():
        raise FileNotFoundError(
            f"Model not found at {args.model_path}. "
            f"Train one first with: python models/turtlebot_recurrent_ppo.py --train --seed {args.seed}"
        )
    ...
    model = RecurrentPPO.load(str(args.model_path), env=eval_venv)
